...
- **Modifier target** (mastery effect target): server-side catalogue entry describing one stat that mastery effects may modify — `key`, `label`, `category`, `allowedModes`, min/max value per mode, `runtimeStatus` (`implemented` | `calculatedOnly` | `notHooked`), `description`. Single source `mastery-effect-targets.ts`, consumed by `sanitizeMasteryEffects` AND exposed via `GET /admin/mastery-effect-targets` (plus `modes` and `contextualStats`); the Studio loads it instead of hardcoding any stat list (saving is blocked if the catalogue fails to load). Ten targets exposed today, all `implemented`. Implemented.
