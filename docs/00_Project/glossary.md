...
- **Respawn**: the return of a dead or exhausted entity to a live state at a designated position. Characters respawn at the nearest `respawn_point`. Animals respawn at their `creature_spawn` position after a delay. Implemented.
...
- **Resource template**: reusable resource type definition with default loot count. Implemented.
...
- **Seed**: startup process that inserts default data (creature templates, resource templates, spawn points). Implemented.
