...
- **Skill**: before 2026-07-09, the codebase used "Skill" (`SkillDefinition`/`PlayerSkill`/`SkillsService`) for what ADR-0018 defines as passive proficiency progression — now renamed to `Mastery` throughout the code, routes, and events. Since the rename, "Skill" is reserved exclusively for the Planned, not-yet-implemented active-ability concept (ADR-0018 §7: cooldown, cost, range, effects). Always qualify when reading older material: a pre-2026-07-09 reference to "skill XP" or "skill definition" means `Mastery`.
