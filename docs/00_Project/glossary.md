...
- Depends on: docs/README.md, docs/ROADMAP.md, docs/08_Gameplay/README.md, docs/01_Architecture/adr/ADR-0001-world-coordinate-system.md, docs/01_Architecture/adr/ADR-0002-entity-positioning.md, docs/01_Architecture/adr/ADR-0020-mastery-contextual-effects.md
...
- **Crafting recipe**: server-side recipe that consumes ingredients, may produce results, grants mastery XP, and declares a `stationType`. Recipes with `stationType = "none"` do not require a station. Recipes with any other `stationType` require a compatible nearby crafting station. Implemented.
- **Crafting station template**: reusable station definition with `key`, `name`, `stationType`, `category`, optional `requiredMasteryKey`, `interactionRadiusWU`, and `enabled`. Implemented as `CraftingStationTemplate`.
...
- **Mastery**: passive/proficiency progression (weapon, armor, profession, gathering, social/leadership) — never an actively-used ability. Defined by ADR-0018. Backed by `MasteryDefinition` (catalogue, key/name/category/maxLevel/baseXpPerLevel/xpCurveExponent) and `PlayerMastery` (per-character level/xp), managed by `MasteriesService`. XP is granted via `applyMasteryXpInTx`, computed by `calculateMasteryXp` (`MasteryXpContext`/`MasteryDomain`), and broadcast to the connected client via the `mastery_update` socket event. Exposed read-only to the player via `GET /characters/me/masteries`; edited by admins via `/admin/mastery-definitions` (DevTools "Mastery Editor"). Implemented — this is what the codebase previously called "Skill" (`SkillDefinition`/`PlayerSkill`/`SkillsService`/`skill_update`) before the ADR-0018 vocabulary alignment (2026-07-09); the rename is code/route/event-level only, the underlying mechanism is unchanged. See ADR-0016 (mechanism) and ADR-0018 §4–5 (vocabulary, categories). Since Masteries V1-D, a `MasteryDefinition` also carries `effects` (contextual bonuses) — see `Mastery effects` below and ADR-0020.
- **Skill**: an actively-used player ability, distinct from `Mastery` (passive proficiency) and `Talent` (out of scope). Partially Implemented (ADR-0019 V1, bounded): `skill_definition` catalogue (key/name/enabled/skillKind/autoUnlock, requirements incl. `requiredMasteries`, resource cost, cooldown, range/radius, targetMode, effectType damage|heal, `scaling` coefficients, nullable `weaponType`), server-side cast (`SkillCastService` — validation, costs, cooldowns, pure `calculateSkillEffect`), damage-on-creature and self-heal paths, per-character action bar, unlock model, Studio Skill Editor. The full ADR-0018 §7 vision (multi-target, buffs, auras runtime, talents) remains Planned concept. Do not confuse with the pre-2026-07-09 codebase usage of "skill" for what is now `Mastery`.
- **Mastery effects** (`mastery_definition.effects`): progressive server-side stat modifiers carried by a mastery definition (ADR-0020, amended V2). JSONB `{ context?, modifiers[] }`, strict whitelist on write / defensive read; `{}` = no effect. Each modifier is `{ stat, mode, value }` — modes `percentPerLevel` (0–5/level) and `flatPerLevel` (0–100/level). Formula: `bonus = level × coefficient` — **masteries start at level 0** (level 0 = no bonus, level 3 × 5 = 15%); totals clamped per stat at 50% (percent) / 1000 (flat). Modifiers WITHOUT context apply permanently through `CharacterStatsCalculator.compute`; modifiers WITH `context.weaponType` apply only on weapon-based combat hooks (stat `physicalAttack` only). Resolved exclusively server-side by `MasteryEffectsService` — the client and the Studio never compute a bonus. Targetable stats come from the single server source exposed by `GET /admin/mastery-effect-targets` (see `Modifier target`). Crit/dodge/parry/block/accuracy/speeds/resistances/stun/craft are Planned concept — deliberately absent from the server targets until their gameplay hooks exist. Implemented.
- **Modifier target** (mastery effect target): server-side catalogue entry describing one stat that mastery effects may modify — `key`, `label`, `category`, `allowedModes`, min/max value per mode, `runtimeStatus` (`implemented` | `calculatedOnly` | `notHooked`), `description`. Since V3-B the targets are **built from the `DerivedStatDefinition` records** (no static list): a stat is exposed iff `enabled` AND `masteryEligible` AND `runtimeStatus === 'implemented'` AND `allowedModifierModes` non-empty. Builder `mastery-effect-targets.ts`, consumed by `sanitizeMasteryEffects` AND exposed via `GET /admin/mastery-effect-targets` (plus `modes` and `contextualStats`); the Studio loads it instead of hardcoding any stat list (saving is blocked if the catalogue fails to load). Thirteen targets exposed today (including `armorPenetrationPercent` V4-B0, `criticalChance`/`criticalDamage` V4-D), all `implemented`. Implemented.
- **Derived stat** (`DerivedStatDefinition`, French "stat secondaire/dérivée"): a server-computed character statistic defined by config (`key` immutable, `label`, `category`, `enabled`, `baseValue`, `minValue`/`maxValue`, `primaryCoefficients`, `masteryEligible`, `allowedModifierModes`, `runtimeStatus`, `description`). Value = `baseValue + Σ(coefficient × final primary stat)`, clamped, then mastery/equipment modifiers applied — always server-side, never recomputed by the client. Edited in the Studio "Stats secondaires" module; served read-only to the player catalogue via `GET /characters/stat-definitions`. **System** derived stats (seeded by code, `DEFAULT_DERIVED_STAT_DEFINITIONS`) are never deletable; **custom** ones (created in the Studio) are deletable only when unreferenced (see maintenance: references report, mastery-reference removal, key duplication). Implemented (V3).
- **`runtimeStatus`**: informative flag on a derived stat — `implemented` (computed AND consumed by at least one gameplay hook), `calculatedOnly` (computed/displayed but not necessarily used), `notHooked` (defined without gameplay effect). Only `implemented` derived stats are exposed as mastery `Modifier target`s. Does not affect computation (only `enabled` does). Implemented.
- **`armorPenetrationPercent`**: system offensive derived stat (V4-B0) that ignores a **percentage** of the target's armor for a **physical** hit: `effectiveArmor = round(targetArmor × (1 - armorPenetrationPercent / 100))` (clamped 0–100, never negative), then `finalDamage = max(0, rawDamage - effectiveArmor)`. It does not lower the target's global armor and creates no persistent debuff — it is a property of the attacker/hit. Does **not** apply to `raw` damage. Targetable as a **permanent** mastery effect (never contextual `weaponType`); consumed by player auto-attack and physical damage skills. Non-deletable (system stat). Implemented.
- **`defensePenetration`**: legacy V4-A stat — a **flat** penetration model (`effectiveDefense = max(0, defense - value)`), superseded by `armorPenetrationPercent`. Kept for compatibility only, **not an active mastery target** (`masteryEligible=false`, `runtimeStatus=calculatedOnly`, no modes); no longer consumed by combat. Deprecated.
- **Physical damage** (`DamageType 'physical'`): default combat damage type; mitigated by the target's armor/defense, reduced by the attacker's `armorPenetrationPercent`. Implemented.
- **Raw damage** (`DamageType 'raw'`): combat damage type that **ignores armor and armor penetration** entirely (`finalDamage = rawDamage`). Available in the pure calculator (`calculateCombatDamage`) and selectable per skill via `damageType`. Implemented.
- **Skill damage type** (`SkillDefinition.damageType`, V4-C): server-side property of a damage skill — `physical` (default; mitigated by armor, uses the caster's `armorPenetrationPercent`) or `raw` (ignores armor and penetration). Validated `IN ('physical','raw')`, backfilled to `physical` by migration, editable in the Studio Skill Editor, passed through `SkillCastService` → `applySkillDamage` → `calculateCombatDamage`. Only `physical`/`raw` today — no magical/elemental/poison. Ignored for heal skills. Implemented.
- **Damage resolution contract** (V4-D0): the fixed phase order of a combat hit, documented in `docs/08_Gameplay/combat-resolution.md` — (1) server-final stats, (2) attack block → `attackPowerFinal`, (3) defense block → `armorAfterPenetration`, (4/5) physical/raw resolution. Server-authoritative. Guides where future mechanics plug in.
- **`magicSchool`** (Planned — ADR-0022): third combat axis categorizing a magical effect, distinct from `damageType` (mitigation) and `attackDefenseKind` (defensive interactions). Initial schools: `fire · water · air · earth · sacred · poison`. Mandatory for a magic skill, forbidden for `physical`/`raw`. Selects the resistance consumed **only** by damaging magic effects; also tags non-damaging magic (heals, buffs) which resistance never reduces. Chosen per skill, never per class. Planned — cadrage only.
- **Magic resistance** (Planned — ADR-0022): per-school stats (`magicResistanceFire/Water/Air/Earth/Sacred/Poison`) plus a transversal `magicResistanceGlobal` contribution injected into each school's resolution (not a second mitigation). Resolved via the shared pipeline (ADR-0021: contributions → filters → operations → optional caps → rounding → traceable value), **no automatic clamp** (V1 decision). Mitigation is linear in percentage points: `damageAfter = damageBefore × (1 − effectiveResistance/100)`; negative resistance = vulnerability; `≥ 100` is **not** an immunity. Consumed only by damaging magic; never reduces heals/buffs. Not consumed by `raw`. Mitigation (damage application) is **Planned**. Foundation **Implemented**: the canonical `magicResistance*` family exists as `calculatedOnly` derived stats (the former `magicalResistance*` were renamed into it — single family), resolved and summable via `resolveEffectiveMagicResistance` but not yet consumed.
- **Immunity vs invulnerability** (Planned — ADR-0022): explicit **annulation** producing 0 damage **before** the minimum-of-1 rule, distinct from numeric resistance (mitigation). Three levels: **school immunity** (one magic school), **magic immunity** (all schools), **total invulnerability** (blocks `physical`, `magic`, `raw`/`bleed`). Each source carries `scope = all|magic|school`, `school`, `sourceType`, `sourceId`, `tags`; target snapshot exposes `isInvulnerable / isMagicImmune / immuneMagicSchools[] / sources[]`. The **Sacred Shield** grants temporary total invulnerability (never modelled as a resistance nor as 100 %). No magic penetration exists (deliberate gameplay decision). Planned — cadrage only.
- **DoT (damage over time)** (Planned — ADR-0022 / `combat-resolution.md` §12): periodic damage effect. Applied only on a normal impact (block/dodge/parry/total invulnerability/matching immunity prevent it). Ticks cannot be dodged/parried/blocked, never crit, re-read the target's **current** defences but use the caster's **offensive snapshot** frozen at application. Minimum 1 per valid tick with initial damage > 0. Deterministic cadence, no immediate tick, full interval only. **Poison** = `damageType magic`, `magicSchool poison`; **Bleed** = `damageType raw`, `magicSchool null` (ignores armour, magic resistance and magic immunity; only total invulnerability cancels it). Planned.
- **`stackingGroup`** (Planned — `combat-resolution.md` §12.10): grouping key for temporary effects. Same group → a single active contribution (highest `priority`, then oldest application); neutralized effects stay present, keep their duration, and reactivate if the active one disappears. Different groups → cumulative. DoTs from different casters are independent instances. Planned.
- **`removeOnDeath`** (Planned — `combat-resolution.md` §12.14): per-effect flag, **default `true`** — temporary effects (buffs/debuffs/poison/bleed/temporary immunities) are removed on the target's death; respawn is effect-free. `false` (persistent curse, death penalty, lingering wound) is a future non-implemented case. Planned.
- **`healingDone` / `healingReceived`** (Planned — ADR-0022 §13 / §12.2): two distinct healing-modifier families in percentage points — caster's outgoing (`healingDone`) and target's incoming (`healingReceived`). `finalHeal = max(0, floor(heal × (1 + healingDone/100) × (1 + healingReceived/100)))`. Same-family contributions add before their multiplier; the two families multiply together. Resistances never reduce heals. Planned.
- **Combat logout / 30 s maintenance** (Planned — `combat-resolution.md` §12.13): a disconnecting player's entity is kept in the world for 30 s (targetable, takes direct hits and DoT ticks, can die), then removed with its in-memory temporary effects. Reconnecting within the window resumes the same entity (no new spawn, no duplicate); a death during the window is never undone. **Not implemented today** (player state is removed immediately on disconnect, position persisted). Planned.
- **Cleanse vs purge** (Planned — `combat-resolution.md` §12.17): two distinct dispel mechanics keyed on `polarity` — **cleanse** (defensive) removes only `harmful` effects; **purge** (offensive) removes only `beneficial` effects. No implicit universal dispel; magic resistances never affect dispel success (dispel is deterministic). Planned.
- **Effect bundle** (Planned — `combat-resolution.md` §12.18): a linked pair `buff + curse` sharing `effectBundleId` and `linkPolicy = remove_bundle` — removing one component removes the whole bundle (prevents keeping the bonus after cleansing the curse). Passive equipment bundles are not individually dispellable; unequipping removes the whole bundle. Both components stay separate in the resolver/Studio trace. Planned.
- **`dispelPriority` / deterministic dispel** (Planned — `combat-resolution.md` §12.19): dispel selection order among compatible effects: polarity → `dispelCategory` → `isDispellable` → highest `dispelPriority` → oldest `appliedAt`. Deterministic (no roll); only `isDispellable = false` blocks removal. `maxEffectsRemoved` default 1 (a bundle counts as one selection); `result = nothing_to_dispel` when a valid cast finds no compatible effect (cost/cooldown still consumed). Planned.
- **`maxConcurrentInstancesPerTarget`** (Planned — `combat-resolution.md` §12.20): per-DoT-definition cap on independent instances from different casters on one target, **default 20**, overridable per creature template / encounter (World Boss not hard-coded). Over the limit → new application refused (`effect_instance_limit_reached`), no existing instance replaced. Planned.
- **Deferred reward** (Planned — `combat-resolution.md` §12.22): a kill caused by a DoT after the caster left the world is still attributed to the initial caster (XP/progression applied transactionally, personal loot right recorded, shared loot follows normal rules). Idempotency key (`killId + characterId + rewardType` or equivalent) prevents double distribution across reconnect / retry / zone transfer. No deferred-reward system built by the current scoping. Planned.
- **`CombatEventsBatch` / `eventAt`** (Planned — `combat-resolution.md` §12.24): authoritative network envelope grouping combat sub-events resolved in one server cycle (`batchId`, `serverTick`, `emittedAt`, `zoneId`, `batchSequence`, `batchCount`, `events[]`). Each `CombatEvent` carries `eventAt` (authoritative resolution instant, distinct from the batch's `emittedAt`), `eventId` (optional client dedup) and, for ticks, `applicationId + tickIndex` (server business idempotency). `maxEventsPerBatch` default 100 → fragmentation over the same `serverTick`. The client only groups display, never computes/reorders. Planned.
- **Zone transfer authority handoff** (Planned — `combat-resolution.md` §12.21): deterministic transfer of a target's temporary runtime state (DoTs, immunities, stacks) between zone/server authorities — source freezes and resolves ticks up to `transferCutoffAt`, target confirms, source drops timers; a due tick during the switch is resolved once (`applicationId + tickIndex`). Per-attempt timeout 3 s, 2 automatic retries, same `transferId`; failure cancels the teleport (no effect dropped, no cadence reset). Compatible with a future distributed architecture without declaring it active. Planned.
- **`maxTotalPeriodicEffectsPerTarget`** (Planned — `combat-resolution.md` §12.20): global cap (default 100) on all periodic effects on a target, on top of the per-definition `maxConcurrentInstancesPerTarget` (default 20). Over the per-definition cap → `effect_instance_limit_reached`; over the global cap → `target_periodic_effect_limit_reached`; refresh of an existing instance is always allowed; no existing instance is replaced. Planned.
- **Personal loot entitlement → mailbox** (Planned — `combat-resolution.md` §12.26): important personal loot becomes an idempotent entitlement (`ground` state, `groundLootDuration` default 5 min); unclaimed after that → atomic transfer to the existing mailbox (`personalLootRetention` default 30 days). Exclusivity: an `entitlementId` is either claimed on the ground or in the mailbox, never both. The persistent entitlement is the authoritative source; the mailbox is the retrieval channel. Character deletion/suspension/ban gate access (pending = kept, permanent deletion = closed + audit, suspension = expiry paused, ban = no retrieval + audit). Planned.
- **`PersonalLootEntitlement`** (Planned — `combat-resolution.md` §12.27): dedicated authoritative entity for a personal loot right; the ground world item and the mailbox letter are only representations. States `ground | mailed | claimed | expired | cancelled` with fixed allowed/forbidden transitions (terminal: claimed/expired/cancelled). Unique key `killId + characterId + rewardRollId` (not `itemId`, since one item may legitimately drop several times). Group/raid: one entitlement **per eligible character** (non-transferable; the leader never picks the recipient). Anti-duplication is enforced server-side with a row lock + conditional state update + idempotent retries + audit; the pickup ⟂ 5-min-expiry race yields exactly one creation (`ground → claimed` **or** `ground → mailed`). Planned.
- **System reward mail channel** (Planned — `combat-resolution.md` §12.27): a reserved mailbox channel for system rewards / personal loot that does **not** consume ordinary mail slots — a personal item never vanishes because the mailbox is full, and a player cannot block their mailbox to extend a loot right. Ordinary player-to-player mail keeps the normal capacity. The mailbox is a retrieval channel, not the authority (the entitlement is). One notification per entitlement (`notifiedAt`); a notification failure never blocks the mail. Planned.
- **Transfer coordinator (`stateVersion` / compare-and-swap)** (Planned — `combat-resolution.md` §12.28): authoritative arbiter of entity ownership during a zone/server transfer, so a permanent source-zone failure produces neither a duplicate nor a lost entity. State `transferId / entityId / sourceZoneId / targetZoneId / status(preparing|accepted|committed|cancelled) / stateVersion / expiresAt`. Failure after `accepted` → ownership granted to the target zone; before `accepted` → cancel + safe restore. A single ownership transition wins via compare-and-swap on `stateVersion`; stale confirmations are rejected; all retries keep the same `transferId`. No Redis/microservice imposed. Planned.
- **`disabledForNewDrops` vs `quarantined`** (Planned — `combat-resolution.md` §12.29): two distinct administrative item states, server-authoritative. `disabledForNewDrops` blocks **only** new drop generation — already-earned entitlements stay claimable, owned items are untouched, no retroactive removal. `quarantined` (exploit/dup/corruption/security) blocks new drops **and** temporarily blocks retrieval of existing entitlements, pauses retention, creates no inventory item, and never silently turns an entitlement into `expired`/`cancelled`; any compensation is a server, idempotent, audited admin operation. Referenced item definitions are never destructively deleted (`active/disabledForNewDrops/quarantined/archived`) so historical references stay resolvable. Planned.
- **System reward mail — no blocking capacity** (Planned — `combat-resolution.md` §12.30): the reserved system reward channel has **no blocking capacity** (persistent storage, UI pagination, per-entitlement expiry); no legitimate entitlement is deleted to free space and no new right is refused merely because of volume. Abnormal-generation protection (detection/audit/metrics/source suspension) never auto-deletes legitimate entitlements. Planned.
- **Notification grouping / `notifiedAt`** (Planned — `combat-resolution.md` §12.30): several entitlements from the same kill may share one summary notification — **visual grouping only** (by `killId + characterId`). Each entitlement keeps its own identity, status, expiry and `notifiedAt`; claiming one never claims the others. `notifiedAt` means "already presented in a notification" — neither `mailed` nor `claimed`; retries never spam; a notification failure never affects the mail or the reward. Planned.
- **Attack block**: the offensive half of a hit — `baseDamage`, the critical multiplier (Implemented, V4-D: `criticalChance`/`criticalDamage`), and future flat/percent damage modifiers, combined into `attackPowerFinal`. Critical belongs here, never in the defense block, and applies before armor. Flat/percent modifiers still Planned.
- **Defense block**: the defensive half of a hit — `defenseTotal`/armor, future flat/percent armor bonuses and reductions (curses), then `armorPenetrationPercent` applied **last** → `armorAfterPenetration`. Partly Planned.
- **`criticalChance`**: server derived stat (V4-D) — chance a physical hit crits, in percent (e.g. `25` = 25 %), clamped 0–100. Attack-block: server roll `[0,1)`, crit if `roll < criticalChance/100` (RNG server-only, injectable in tests). System stat `implemented` + `masteryEligible`, exposed as a **permanent** Mastery Effect target (never contextual `weaponType`). Feeds auto-attack and damage skills. Implemented.
- **`criticalDamage`**: server derived stat (V4-D) — total critical multiplier in percent (e.g. `150` → ×1.5), baseValue 150. On a crit, `attackPowerFinal = round(baseDamage × criticalDamage/100)`. Same status (permanent Mastery Effect target). Implemented.
- **`armorReduction` (flat/percent)**: Planned defense-block **debuff on the target** (curses), distinct from `armorPenetrationPercent` (a property of the attacker/hit). Applied before penetration. Duration/stack rules undecided. Not implemented.
- **`requiredMasteries`**: lock/prerequisite map `{ masteryKey: minLevel }` on items, skills, and recipes. Grants **no bonus** — it only allows or blocks an action (equip, cast, craft). Centralized in `MasteriesService.evaluateRequiredMasteries`. Implemented. Never use it as a bonus proxy (rejected alternative, ADR-0020).
- **`weaponType`**: free-string contract (`[a-z0-9_]`, e.g. `two_handed_sword`, `bow`) shared by `item.weaponType` (what the weapon is), `skill_definition.weaponType` (which weapon a skill is compatible with) and `effects.context.weaponType` (which weapon a mastery effect applies to — a weapon type, never a mastery key). On a skill, `null` = not weapon-based → never receives weapon mastery bonuses; a non-null value does **not** require the weapon to cast. The equipped weaponType is always resolved server-side (`resolveEquippedWeaponType`, ranged > melee priority). Implemented.
- **Weapon-based skill**: an offensive skill (`effectType: damage`, `targetMode: creature`) with a non-null `weaponType`. Receives the contextual `physicalAttack` mastery modifiers (percent + flat) on its final amount only when the equipped weapon matches. Implemented. Heals, self skills, and skills with `weaponType: null` never receive contextual weapon bonuses (they do benefit from permanent modifiers like `healingPower`).
- **Contextual bonus**: a mastery bonus that applies only when the real, server-resolved gameplay context (equipped weapon today; armor, recipe… in the future) matches the mastery's declared `effects.context`. Domains provide context; they never own masteries (ADR-0020).
- **`damagePercentPerLevel`**: legacy V1 mastery effect key (`effects.combat`). Read defensively (interpreted as a `physicalAttack / percentPerLevel` modifier) but **no longer generated** — writes are converted to `effects.modifiers[]` (V2). Deprecated as a written format.
...
- **Building**: a static gameplay entity placed on the map. Implemented as `BuildingTemplate` (type, interaction radius, texture key) and `Building` (instance with position, mapId, state). Integrated into the WOM (category="building", kind="entity"/"definition"). State: `ACTIVE`, `DISABLED`, `LOCKED`, `UNDER_CONSTRUCTION`, `DESTROYED`. Interaction radius validated server-side via `WorldService.validateInteraction` (chebyshevDistanceWU). Opens a `WindowManager` window on click (AuctionHouseWindow, MailboxWindow). Placement and CRUD managed via AdminPanel WOM.
...
- **Auction House**: A settlement market service for fixed-price listings. Implemented: create listing, item escrow (LISTED+AUCTION), buyout via Mailbox pipeline (2 system mails per purchase + `auction_escrow` wallet), cancel → seller mail, expiration → seller mail. UI via `AuctionHouseWindow` (3 tabs: Browse / My listings / Sell). Requires an `auction_house` Building in range. Only `INSTANCE` items can be listed. Distribution via `MailService.sendSystemMailWithinManager` + `AUCTION_TO_MAIL` transition. Future: timed auctions (MVP 2), taxes and Treasury (MVP 3).
- **Auction escrow**: A system wallet (`ownerType: "system"`, `ownerId: "auction_escrow"`) that holds buyer payment between purchase and seller's money mail claim. Funded on purchase (`AUCTION_BUY`), released on mail claim (`AUCTION_SELL`). Known debt: expired money mails leave funds blocked in escrow with no automatic return (Auction MVP 2).
- **Mail / Mailbox**: An asynchronous player messaging system. Implemented: inbox (PENDING mails), system mails from `SYSTEM` sender (generated by Auction pipeline), claim of item attachment (CLAIM_MAIL transition) or money attachment (AUCTION_SELL economy transfer from escrow). UI via `MailboxWindow`. Requires a `mailbox` Building in range for mutations. `MailService.sendSystemMailWithinManager()` is the sole entry point for system mails, always called within the caller's transaction.
...
- **Runtime crafting**: player-facing craft flow from a placed station through `ActionPanel`, compatible recipes, `POST /crafting/craft`, server validation, inventory changes, and mastery XP refresh. Implemented.
...
- **Skill**: before 2026-07-09, the codebase used "Skill" (`SkillDefinition`/`PlayerSkill`/`SkillsService`) for what ADR-0018 defines as passive proficiency progression — now renamed to `Mastery` throughout the code, routes, and events. Since the rename, "Skill" refers exclusively to the active-ability concept, whose bounded V1 is now Implemented (ADR-0019: `skill_definition`, server-side cast, costs/cooldowns, damage/heal, action bar, `weaponType`). Always qualify when reading older material: a pre-2026-07-09 reference to "skill XP" or "skill definition" means `Mastery`.
