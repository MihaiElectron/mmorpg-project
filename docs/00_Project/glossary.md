...
- **Modifier target** (mastery effect target): server-side catalogue entry describing one stat that mastery effects may modify — `key`, `label`, `category`, `allowedModes`, min/max value per mode, `runtimeStatus` (`implemented` | `calculatedOnly` | `notHooked`), `description`. Since V3-B the targets are **built from the `DerivedStatDefinition` records** (no static list): a stat is exposed iff `enabled` AND `masteryEligible` AND `runtimeStatus === 'implemented'` AND `allowedModifierModes` non-empty. Builder `mastery-effect-targets.ts`, consumed by `sanitizeMasteryEffects` AND exposed via `GET /admin/mastery-effect-targets` (plus `modes` and `contextualStats`); the Studio loads it instead of hardcoding any stat list (saving is blocked if the catalogue fails to load). Eleven targets exposed today (adding `defensePenetration` in V4-A), all `implemented`. Implemented.
...
- **`defensePenetration`**: system offensive derived stat (V4-A) that reduces the target's effective defense during physical damage: `effectiveDefense = max(0, targetDefense - defensePenetration)` (never negative), applied inside `calculateCombatDamage`. Targetable as a **permanent** mastery effect (never contextual `weaponType`); consumed by player auto-attack and damage skills. Non-deletable (system stat). Implemented.
