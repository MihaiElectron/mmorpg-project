...
- **Auction House**: A settlement market service for fixed-price listings. MVP 1 implemented: create listing, item escrow, buyout, seller cancellation, expiration scheduler. UI via `AuctionHouseWindow` (3 tabs: Browse / My listings / Sell). Requires an `auction_house` Building in range. Only `INSTANCE` items can be listed. Future: timed auctions (MVP 2), taxes and Treasury (MVP 3).
