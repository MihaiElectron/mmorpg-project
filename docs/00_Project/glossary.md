...
- **Modifier target** (mastery effect target): server-side catalogue entry describing one stat that mastery effects may modify — `key`, `label`, `category`, `allowedModes`, min/max value per mode, `runtimeStatus` (`implemented` | `calculatedOnly` | `notHooked`), `description`. Since V3-B the targets are **built from the `DerivedStatDefinition` records** (no static list): a stat is exposed iff `enabled` AND `masteryEligible` AND `runtimeStatus === 'implemented'` AND `allowedModifierModes` non-empty. Builder `mastery-effect-targets.ts`, consumed by `sanitizeMasteryEffects` AND exposed via `GET /admin/mastery-effect-targets` (plus `modes` and `contextualStats`); the Studio loads it instead of hardcoding any stat list (saving is blocked if the catalogue fails to load). Eleven targets exposed today (including `armorPenetrationPercent`, V4-B0), all `implemented`. Implemented.
...
- **Attack block**: the offensive half of a hit — `baseDamage`, future flat/percent damage modifiers, and the future critical multiplier, combined into `attackPowerFinal`. Critical belongs here, never in the defense block. Partly Planned.
...
- **`criticalChancePercent`**: Planned attack-block stat — chance a hit crits, clamped 0–100. Not implemented.
- **`criticalDamagePercent`**: Planned attack-block stat — critical damage bonus (e.g. `50` → ×1.5). Not implemented.
