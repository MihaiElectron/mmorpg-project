...
- **Magic resistance** (Planned — ADR-0022): per-school stats (`magicResistanceFire/Water/Air/Earth/Sacred/Poison`) plus a transversal `magicResistanceGlobal` contribution injected into each school's resolution (not a second mitigation). Resolved via the shared pipeline (ADR-0021: contributions → filters → operations → optional caps → rounding → traceable value), **no automatic clamp** (V1 decision). Mitigation is linear in percentage points: `damageAfter = damageBefore × (1 − effectiveResistance/100)`; negative resistance = vulnerability; `≥ 100` is **not** an immunity. Consumed only by damaging magic; never reduces heals/buffs. Not consumed by `raw`. Planned — the current `magicalResistanceFire/Water/Air/Earth` player stats are `calculatedOnly` (computed, not consumed).
