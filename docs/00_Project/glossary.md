...
- **Mastery effects** (`mastery_definition.effects`): contextual, progressive server-side bonuses carried by a mastery definition (ADR-0020). JSONB, strict whitelist on write / defensive read; `{}` = no effect. Formula: `bonus = (level − 1) × percentPerLevel`, level 1 = 0%, total damage percent clamped at 50%. Resolved exclusively server-side by `MasteryEffectsService` — the client and the Studio never compute a bonus. Implemented for `context.weaponType` + `combat.damagePercentPerLevel` only (consumed by auto-attack and weapon-based skills). Crit/penetration/stun/knockback/block/armor mitigation/craft success-quality are Planned concept — deliberately absent from the server whitelist until their gameplay hooks exist.
...
- **`weaponType`**: free-string contract (`[a-z0-9_]`, e.g. `two_handed_sword`, `bow`) shared by `item.weaponType` (what the weapon is) and `skill_definition.weaponType` (which weapon a skill is compatible with). On a skill, `null` = not weapon-based → never receives weapon mastery bonuses; a non-null value does **not** require the weapon to cast. The equipped weaponType is always resolved server-side (`resolveEquippedWeaponType`, ranged > melee priority). Implemented.
- **Weapon-based skill**: an offensive skill (`effectType: damage`, `targetMode: creature`) with a non-null `weaponType`. Receives the mastery `damagePercentPerLevel` bonus on its final amount only when the equipped weapon matches. Implemented. Heals, self skills, and skills with `weaponType: null` never receive weapon mastery bonuses.
...
- **`damagePercentPerLevel`**: the only mastery effect key implemented today (`effects.combat`). Percent of damage added per mastery level above 1, bounded 0–5 at write time, total clamped at 50% at read time. Consumed by auto-attack and weapon-based skills. Implemented.
