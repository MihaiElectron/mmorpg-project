...
- **Raw damage** (`DamageType 'raw'`): combat damage type that **ignores armor and armor penetration** entirely (`finalDamage = rawDamage`). Available in the pure calculator (`calculateCombatDamage`); the `SkillDefinition` model does not yet carry a damage type, so skills are physical by default. Implemented (calculator) / Planned (skill model).
