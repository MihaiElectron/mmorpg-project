...
The observed tool can display overview data, creatures, resources, and characters; update selected fields; spawn creatures and resources; teleport characters; move creatures; delete creatures or resources; and force creature respawn by template.
...
| Admin HTTP controller | `/admin/*` in `apps/api-gateway/src/admin/admin.controller.ts` | Exposes overview, templates, spawns, creatures, characters, resources, and resource templates. | `JwtAuthGuard`, `RolesGuard`, `@Roles(UserRole.ADMIN)`. | Implemented |
...
- Fetch overview, creature templates, creatures, resource templates, resources, and characters through HTTP.
...
- Delete creature and resource instances through admin Socket.IO events.
...
| Load live creatures | `AdminPanel.tsx` | `GET /admin/creatures` | `JwtAuthGuard`, `RolesGuard`, `@Roles(UserRole.ADMIN)` | Implemented |
...
| --- | --- | --- | --- | --- | --- |
| `admin:spawn` | `commandRegistry.ts`, `AdminPanel.tsx` drag | `AdminGateway.onSpawn` | `client.data.role === 'admin'` | Creates an admin creature spawn and creature, then emits `creature_update`. | Implemented |
...
| `admin:move_creature` | `commandRegistry.ts` | `AdminGateway.onMoveCreature` | `client.data.role === 'admin'` | Moves a live creature through `CreaturesService`. | Implemented |
| `admin:update_template` | `commandRegistry.ts`, `AdminPanel.tsx` | `AdminGateway.onUpdateTemplate` | `client.data.role === 'admin'` | Updates creature template fields (including `respawnDelayMs`), refreshes in-memory creature template data, emits `category:updated`. | Implemented |
...
| `admin:update_creature` | `AdminPanel.tsx` | `AdminGateway.onUpdateCreature` | `client.data.role === 'admin'` | Updates creature state, health, coordinates, or per-instance `respawnDelayMs`. | Implemented |
| `admin:delete_creature` | `AdminPanel.tsx`, `ActionPanel.tsx` | `AdminGateway.onDeleteCreature` | `client.data.role === 'admin'` | Deletes an creature through `CreaturesService` and emits a dead `creature_update`. | Implemented |
...
| `admin:respawn_all` | `commandRegistry.ts` | `AdminGateway.onRespawnAll` | `client.data.role === 'admin'` | Forces all matching creatures for a template back to spawn state. | Implemented |

## Available admin actions

| Action | Transport | Target | Main effect | Risk if repeated | Status |
| --- | --- | --- | --- | --- | --- |
| View overview | HTTP | Admin dashboard counters | Reads template, spawn, and active creature counts. | Low; read-only. | Implemented |
| View creatures | HTTP | Creature templates and creature instances | Reads editable creature data. | Low; read-only. | Implemented |
...
| `/spawn <template> [x] [y]` | Socket.IO | Creature template and coordinates | Creates a new admin creature spawn and creature. | May create duplicate spawns or creatures; idempotence is Not verified. | Implemented |
| Drag creature group to map | Socket.IO | Creature template and drop coordinates | Creates a new creature spawn and creature. | May create duplicate spawns or creatures; idempotence is Not verified. | Implemented |
...
| `/tp [id] <x> <y>` | Socket.IO | Player id/name or selected creature | Teleports a connected player or moves a selected creature. | Repeated command can move the target repeatedly; destination bounds are Not verified. | Implemented |
...
| `/respawn all <template>` | Socket.IO | Creatures for one template | Resets matching creatures to spawn state. | Mass-operation idempotence and replay protection are Not verified. | Implemented |
...
| Apply creature instance fields | Socket.IO | Creature instance | Updates state, health, x, y, or per-instance `respawnDelayMs`. | Repeated updates overwrite instance values. | Implemented |
...
| Delete creature | Socket.IO | Creature instance | Deletes creature and emits dead update. | Repeated delete becomes target-missing behavior; audit is Not verified. | Implemented |
...
- Overview counters: templates, spawns, active creatures.
...
Observed Socket.IO payload checks include required ids or keys, numeric coordinate checks, allowed-field lists for template, character, resource, and creature updates, and allowed state values for creature and resource state changes.
...
- `AdminPanel` fetches `/admin/overview`, `/admin/creatures`, `/admin/templates`, `/admin/resource-templates`, `/admin/resources`, and `/admin/characters`.
- `AdminPanel` subscribes to `creature_update` and `resource_update`.
