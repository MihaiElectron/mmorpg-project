...
| `animal` | `x`, `y` (INT) | `worldX`, `worldY`, `mapId` | ✅ WU écrits au runtime (session 3) — `x/y` cache pixel |
...
`AnimalsGateway` et `ResourcesGateway` accèdent à `player.worldX/Y` pour les calculs de portée. `player.x/y` reste présent comme cache pixel (non utilisé pour la logique métier).
...
- `wuToIsoScreenX/Y(worldX, worldY)` — pixel cache dérivé depuis WU (`world.service.ts`, `animals.service.ts`, `admin.service.ts`)
...
### ~~`animals.service.ts` — entièrement non migré~~  — ✅ SOLDÉ (session 3)
...
| `attack_animal` | `{ targetId }` | `player.worldX/Y` depuis `ConnectedPlayer` pour le range check ✅ |
...
~~**C2. `animals.service.ts` — boucle IA entièrement en pixels**~~ — **SOLDÉ** (session 3). Boucle IA WU-authoritative, `findNearestPlayer` WU + filtre mapId, `attack()` en WU.
...
~~**I1. Animaux — worldX/Y jamais écrits au runtime**~~ — **SOLDÉ** (session 3). `animals.service.ts` écrit `worldX/Y/mapId` à chaque mouvement. Pixel cache dérivé de WU.
...
- [x] **Migrer `animals.service.ts`** : boucle IA WU-authoritative (session 3).
- [x] **Double-écriture animaux** : `animals.service.ts` écrit `worldX/Y/mapId` (session 3).
...
- [ ] **Drop colonnes legacy DB** : `character.positionX/Y`, `animal.x/y` — migrations TypeORM à écrire.
...
| Backend — animals.service | Boucle IA WU, double-écriture ✅ | 100% |
...
| Drop colonnes legacy DB | positionX/Y, animal.x/y en cache | 0% |
| **Global estimé** | | **~95%** |
