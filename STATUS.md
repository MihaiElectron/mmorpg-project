...
| — | Module Studio « Stats / Dérivées » dédié (édition des `DerivedStatDefinition` + visualisation des sources de modificateurs) non implémenté | Low | Studio futur |
...
1. **Effets de maîtrise futurs** — critique, pénétration, stun/knockback, block, mitigation armure, succès/qualité craft — au rythme des hooks serveur combat/craft (ajout aux `mastery-effect-targets` quand le hook existe, ADR-0020)
2. **Module Studio « Stats / Dérivées »** — édition des formules `DerivedStatDefinition` et visualisation des sources de modificateurs
