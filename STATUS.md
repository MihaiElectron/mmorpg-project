...
**Contrat de résolution combat posé V4-D0 (doc)** : `docs/08_Gameplay/combat-resolution.md` fige l'ordre de résolution d'un hit en phases séparées — (1) stats finales serveur, (2) bloc attaque (`attackPowerFinal`, critique futur inclus ici), (3) bloc défense (`armorPenetrationPercent` appliqué **en dernier**, après bonus/malus d'armure futurs), (4/5) résolution physical/raw. Règle clé documentée : la pénétration ignore un % de l'armure **restante** (elle ne modifie pas l'armure globale, n'est pas un debuff). Critique (`criticalChancePercent`/`criticalDamagePercent`), curses/`armorReduction`, block/esquive/résistances documentés comme **Planned** (points d'insertion fixés). Aucun code changé. Doc seule.
