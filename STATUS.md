...
**CraftJob Phase 2 (completion scheduler)** : `CraftJobScheduler` (`@Cron` chaque minute, batch borné) complète les jobs RUNNING échus via `CraftJobService.complete()` — transaction unique, verrou + recheck RUNNING (idempotent), succès/échec depuis le snapshot, XP perso + skill à `RUNNING→COMPLETED` (ADR-0016), consommation des ingrédients INSTANCE réservés (`CONSUME_FROM_CRAFT_ORDER → DESTROYED`). **Le scheduler ne crée jamais d'item** (output figé en quantités `resolvedQuantity`). **Limite V1 : résolution atomique** (toute la `quantity` résolue d'un coup à `finishAt`, pas de progression par unité) → **pas d'annulation partielle** ; `CANCELLED` (à venir) depuis RUNNING = remboursement total, `FAILED` ne rembourse pas (ADR-0009 Décision 3bis).
**CraftJob Phase 3 (claim)** : `CraftJobService.claim(characterId, jobId)` — transaction unique, verrou + revalidation (owner, `COMPLETED`, pas déjà `CLAIMED`), matérialise les outputs depuis le **snapshot** (`resolvedQuantity`) via `ItemMaterializationService.materialize()` (source `CRAFT`, dest INVENTORY — objets identiques au craft instantané), puis `state=CLAIMED`. **`claim` est le seul endroit CraftJob appelant `ItemMaterializationService`** (absent de launch/complete/scheduler). Idempotent (2ᵉ claim → 409), **`FAILED` non claimable**. **`CANCELLED` et la restitution des ingrédients réservés non consommés restent non implémentés** (Phase suivante).
**CraftJob Phase 4 (API + UI Runtime)** : endpoints `POST /crafting/jobs` (launch), `GET /crafting/jobs` (liste du joueur), `POST /crafting/jobs/:jobId/claim` (claim) — `characterId` résolu serveur, délégation pure à `CraftJobService`. `CraftingRuntimePanel` (ActionPanel) : second onglet **Production** — Productions en cours (barre `<progress>` + temps restant), Terminées (bouton **Réclamer**), Échouées (succès/échecs) ; recette détail = deux boutons **Fabriquer maintenant** (instantané, inchangé) / **Production** (launch). **Claim utilisable en jeu**, refresh auto (jobs + inventaire + skills + perso) après launch/claim sans recharger la scène. **Polling simple 10 s** (remplaçable par websocket). **DTO 100 % snapshot** : `recipeName` snapshoté au launch (`craft_job.recipeName`), nom/image d'output via catalogue `Item` par `itemId` du snapshot — **affichable même si la recette est renommée/désactivée/supprimée** (jamais de lecture de la recette vivante). **`CANCELLED` / refund toujours non implémentés.**
...
| Crafting instantané | Stations en WU, validation distance serveur, UX produit-first (recherche/quantité/ingrédients possédé-requis), ingrédients STACKABLE + INSTANCE (`CRAFT_CONSUME`), XP perso (`ProgressionService`) + XP skill Runtime (`calculateSkillXp`) en transaction unique |
| CraftJob Runtime V1 (fondation) | Entités `craft_job` / `craft_job_ingredient` / `craft_job_output` ; `CraftJobService.launch()` crée un job **RUNNING uniquement** avec snapshot immuable complet (recipeVersion/jobVersion/serverFormulaVersion) et **réserve les ingrédients** (escrow : STACKABLE décrément `Inventory`, INSTANCE via transition `RESERVE_FOR_CRAFT` → `IN_CRAFT_ORDER/CRAFT_ORDER`). **Pas de scheduler, pas de claim, pas de matérialisation d'output** — un job ne se termine jamais automatiquement (ADR-0009). |
