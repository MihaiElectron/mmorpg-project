...
| Défense créature (V6-B3→B6) | Chaîne défensive complète : esquive (`dodgeChance`, réduite par accuracy), blocage (`blockChance`/`blockReductionPercent`, physical après armure), typage `attackDefenseKind` physical/magic (**parabilité par la nature** : physical parable même si `damageType: raw` ; magic non parable ; ranged physique parable), parade (`parryChance`, prioritaire, `isParried` propagé). `raw` = mitigation (ignore armure/résistance/blocage à l'impact), **pas** une nature défensive (`isAttackParryable` aligné, `d81ea80`). Inspector aligné. `counterAttackPower`/`maxHealthDerived` informatifs. Seul le modèle hybride physical+raw parallèle reste Planned. Détail : `docs/08_Gameplay/combat-resolution.md` §11.7 |
