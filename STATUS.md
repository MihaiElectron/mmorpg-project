...
**defensePenetration V4-A — premier hook combat via stat dérivée** : `defensePenetration` ajoutée comme **stat dérivée système** offensive (`masteryEligible`, `runtimeStatus: implemented`, modes percent/flat), seedée non destructivement (`DerivedStatsService.seedMissingDefaults` + migration idempotente `AddDefensePenetrationDerivedStat`). Ciblable en **modificateur de maîtrise permanent** (jamais contextuel `weaponType`). Consommée par l'**auto-attaque** et les **skills damage** : `calculateCombatDamage` applique `effectiveDefense = max(0, targetDefense − attackerDefensePenetration)` (jamais négative) avant le calcul de dégâts ; pénétration 0 = comportement inchangé. Combat joueur validé inchangé hors ce hook. Commit : 5985c0b.
**Dégâts admin créature — Directs vs Combat simulé (V4)** : socket **`admin:damage_creature`** avec deux modes clairs — **Directs** (retrait de PV bruts, défense ignorée ; maintenance/debug HP, comportement historique) et **Combat simulé** (applique la défense de la cible via `calculateCombatDamage`, **pénétration admin optionnelle**). UI dans le panneau créatures (aide « Directs ignore la défense. Combat simulé applique la défense de la cible. »). Réutilise le calculateur pur (aucune formule parallèle), serveur autoritaire, combat joueur non impacté. Commit : 83a0c58.
...
1. **Effets de maîtrise futurs** — critique, stun/knockback, block, mitigation armure, succès/qualité craft — au rythme des hooks serveur combat/craft (ajout aux `mastery-effect-targets` quand le hook existe, ADR-0020). _`defensePenetration` livrée (V4-A)._
