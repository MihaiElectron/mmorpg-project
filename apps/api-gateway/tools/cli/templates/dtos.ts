...
function buildDtoImports(
  ...
  const validatorImport = `import { ${Array.from(validators)
    .sort()
    .join(', ')} } from 'class-validator';`;
  const transformerImport =
    transformers.size > 0
      ? `import { ${Array.from(transformers)
          .sort()
          .join(', ')} } from 'class-transformer';`
      : '';
...
function buildCreateRelationDtoField(field: RelationField): string {
  const lines: string[] = [];
  const targetIdName =
    field.relationType === 'many-to-one' || field.relationType === 'one-to-one'
      ? `${field.name}Id`
      : `${field.name}Ids`;
