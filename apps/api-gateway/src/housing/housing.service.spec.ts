...
function makeHousesRepo(house: House | null = makeHouse()) {
  return { findOneBy: jest.fn().mockResolvedValue(house) };
}
...
function makeManager(instance: ItemInstance | null) {
  return { findOne: jest.fn().mockResolvedValue(instance) } as unknown as EntityManager;
}

function buildService(
  instance: ItemInstance | null,
  house: House | null = makeHouse(),
  storedInstances: ItemInstance[] = [],
  items: Item[] = [makeItem()],
) {
  const housesRepo = makeHousesRepo(house);
  ...
  const service = new HousingService(
    housesRepo as unknown as Repository<House>,
  ...
  return { service, itemTransfer, dataSource };
}
...
describe("HousingService", () => {
  ...
  describe("listContents", () => {
    it("retourne les instances stockees dans la maison", async () => {
      const inst = makeInstance({
        state: ItemInstanceState.IN_HOUSING,
        containerType: ItemInstanceContainerType.HOUSING,
        containerId: "house-1",
      });
      const { service } = buildService(null, makeHouse(), [inst]);
      const result = await service.listContents("owner-1", "house-1");
      expect(result).toHaveLength(1);
      expect(result[0].instanceId).toBe("inst-1");
    });

    it("retourne un tableau vide si aucune instance", async () => {
      const { service } = buildService(null, makeHouse(), []);
      const result = await service.listContents("owner-1", "house-1");
    ...
    it("leve ForbiddenException si le personnage n est pas proprietaire", async () => {
      const { service } = buildService(null, makeHouse());
      await expect(service.listContents("non-owner", "house-1")).rejects.toBeInstanceOf(ForbiddenException);
    });

    it("leve NotFoundException si la maison n existe pas", async () => {
  ...
  describe("deposit", () => {
    it("appelle STORE_HOUSE via ItemTransferService", async () => {
      const instance = makeInstance();
      const { service, itemTransfer } = buildService(instance);

      await service.deposit("owner-1", "house-1", "inst-1");

      expect(itemTransfer.transfer).toHaveBeenCalledWith(
        expect.anything(),
        "inst-1",
        expect.objectContaining({ transition: { type: "STORE_HOUSE", houseId: "house-1" } }),
      );
    });

    it("leve NotFoundException si instance inexistante", async () => {
      const { service } = buildService(null);
      await expect(service.deposit("owner-1", "house-1", "ghost")).rejects.toBeInstanceOf(NotFoundException);
    });

    it("leve ForbiddenException si personnage non proprietaire de la maison", async () => {
      const { service } = buildService(makeInstance());
      await expect(service.deposit("non-owner", "house-1", "inst-1")).rejects.toBeInstanceOf(ForbiddenException);
    });
    ...
    it("refuse si l instance n appartient pas au personnage", async () => {
      ...
      await expect(service.deposit("owner-1", "house-1", "inst-1")).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it("refuse le double depot (deja en housing)", async () => {
      const instance = makeInstance({
        state: ItemInstanceState.IN_HOUSING,
        containerType: ItemInstanceContainerType.HOUSING,
        containerId: "house-1",
  ...
  describe("withdraw", () => {
    it("appelle WITHDRAW_HOUSE via ItemTransferService", async () => {
      const instance = makeInstance({
        state: ItemInstanceState.IN_HOUSING,
        containerType: ItemInstanceContainerType.HOUSING,
        containerId: "house-1",
      ...
      await service.withdraw("owner-1", "house-1", "inst-1");

      expect(itemTransfer.transfer).toHaveBeenCalledWith(
        expect.anything(),
        "inst-1",
        expect.objectContaining({
          transition: { type: "WITHDRAW_HOUSE", houseId: "house-1", characterId: "owner-1" },
        }),
      );
    });
    ...
    it("leve NotFoundException si instance inexistante", async () => {
      const { service } = buildService(null);
      await expect(service.withdraw("owner-1", "house-1", "ghost")).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it("leve ForbiddenException si personnage non proprietaire de la maison", async () => {
      ...
      const { service } = buildService(instance);
      await expect(service.withdraw("non-owner", "house-1", "inst-1")).rejects.toBeInstanceOf(ForbiddenException);
    });

    it("refuse si l instance n est pas dans cette maison", async () => {
      const instance = makeInstance({
        state: ItemInstanceState.IN_HOUSING,
        containerType: ItemInstanceContainerType.HOUSING,
        containerId: "autre-house",
      });
      const { service } = buildService(instance);
      await expect(service.withdraw("owner-1", "house-1", "inst-1")).rejects.toBeInstanceOf(BadRequestException);
    });

    it("refuse si l instance n est pas en housing (double retrait)", async () => {
